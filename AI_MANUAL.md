...
| `mcp_server.py` | 444 | MCP stdio 服务，通过 HTTP 代理调用 Flask | 13 个 MCP 工具，覆盖项目、图、文件、反向链接、断链、历史、扫描和同步建议 |
...
GET  /api/files
  返回 [{path, type, exists, size, last_modified, link_count, backlink_count}]

GET  /api/file/<path:file_path>
  返回 {path, dependencies, type?, exists?, size?, abs_path?, last_modified?, links?, issues}
  links: [{target, anchor, context}]
  dependencies: {path, references: [{path, type, link_type}], referenced_by: [{path, type, link_type}]}
...
GET  /api/broken-links
  返回 {broken_links: [{file, target, link_type, context}], count}

GET  /api/graph
  返回 {nodes: [{id, label, group, title, is_external, mounted}], edges: [{from, to, label, title, is_external}]}
...
4. **空格可以存在**，但建议避免 `<`、`>`、`"`、`|`、`?`、`*`。
5. **内部目标文件不存在时是断链**，`/api/file/<path>` 和 `/api/broken-links` 会报告内部断链。
