...
async function addProjectRoot(root){if(!root)return;try{const resp=await fetch('/api/projects/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({root})});const data=await resp.json();if(data.success){await loadProjects();if(data.project)await selectProject(data.project.name);}else{alert('添加项目失败：'+JSON.stringify(data));}}catch(e){alert('添加项目失败：'+e.message);}}
async function addProject(){try{const path=await pickFolder();if(path)await addProjectRoot(path);return;}catch(e){}const picker=document.getElementById('folder-picker');picker.value='';picker.click();}
async function handleFolderPicked(){const picker=document.getElementById('folder-picker');const files=picker.files;if(!files||files.length===0)return;const root=prompt('请输入项目文件夹的完整路径：','');if(!root)return;await addProjectRoot(root);}
