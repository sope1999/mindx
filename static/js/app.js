...
function renderMemoryRefTree(container){
  ...
  const opts={layout:{hierarchical:{enabled:isFirstRender,direction:'UD',sortMethod:'directed',levelSeparation:100,nodeSpacing:70,treeSpacing:150,blockShifting:true,edgeMinimization:true}},physics:{enabled:true,solver:'forceAtlas2Based',forceAtlas2Based:{gravitationalConstant:-50,centralGravity:0.01,springLength:100,springConstant:0.08},stabilization:{iterations:300}},interaction:{hover:true,navigationButtons:true,keyboard:true}};
...
function showConfirmDelete(name){const msg='确定要删除项目"'+name+'"吗？删除后可从项目列表重新添加。';document.getElementById('modal-confirm-msg').textContent=msg;document.getElementById('modal-confirm').style.display='flex';document.getElementById('modal-confirm-ok').onclick=async()=>{document.getElementById('modal-confirm').style.display='none';await removeProject(name);};}
function showProjectError(name,path){document.getElementById('modal-error-msg').textContent='路径不存在：'+path+'，请重新选择文件夹或删除项目';document.getElementById('modal-error').style.display='flex';window._errorProjectName=name;}
