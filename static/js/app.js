...
function getGroupColor(ft){const isLight=document.documentElement.getAttribute('data-theme')==='light';if(isLight){const c={root_index:'#d4e2fc',constitution:'#fde8d0',project_index:'#d4f5d4',project_overview:'#d4f5d4',project_progress:'#c8f0c8',dev_sessions:'#d4e2fc',dev_sessions_old:'#e0e3e8',tool_l2:'#e6d9fc',tool_l3:'#f0e4fc',tool_standalone:'#e4d9f7',archive_index:'#e0e3e8',diary:'#e8e8e8',cheatsheet:'#fde8d0',manual:'#fde8d0',bookmarks:'#fde8d0'};return c[ft]||'#e8e8e8';}const c={root_index:'#4f7fd6',constitution:'#d36b24',project_index:'#2ea043',project_overview:'#2ea043',project_progress:'#1f883d',dev_sessions:'#4f7fd6',dev_sessions_old:'#66758a',tool_l2:'#8250df',tool_l3:'#a371f7',tool_standalone:'#8957e5',archive_index:'#66758a',diary:'#545d68',cheatsheet:'#d36b24',manual:'#d36b24',bookmarks:'#d36b24'};return c[ft]||'#545d68';}
...
function getNodeColor(ftype,path){const cls=getClassification(path);const isLight=document.documentElement.getAttribute('data-theme')==='light';if(cls==='base')return isLight?'#d4e2fc':'#4f7fd6';if(cls==='standalone')return isLight?'#fde8d0':'#d36b24';if(cls==='external')return isLight?'#d3f0f4':'#18a6b8';return getGroupColor(ftype);}
...
function renderDepGraph(){
  ...
  const nodesArr=gNodes.map(n=>{const base={id:n.id,label:baseName(n.id),group:n.group,title:n.title,color:{background:getNodeColor(n.group,n.id),border:getNodeBorderColor(),highlight:{background:getNodeColor(n.group,n.id),border:'#fff'}},font:{color:getFontColor(),size:11,face:'monospace'},shape:'box',margin:5};const cls=getClassification(n.id);if(cls in clsCount){const idx=CLS.indexOf(cls);base.x=(clsIndex[n.id]-(clsCount[cls]-1)/2)*X_GAP;base.y=Y_TOP+idx*Y_GAP;}if(n.is_external){base.shapeProperties={borderDashes:[5,5]};base.borderWidth=2;}return base;});
