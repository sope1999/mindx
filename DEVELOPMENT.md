...
| # | 版本 | 现象 | 根因 | 修复 |
|---|------|------|------|------|
| 30 | v4.6 | 互相引用的节点在引用树图中掉到 level 0 | Kahn 拓扑排序遇到 mutual-reference cycle 时没有给循环内节点保留最佳层级 | 循环节点使用 `bestLevel`，不再统一回退到 0 |
| 31 | v4.6 | 项目排除目录里的文件仍被扫描和解析 | `excluded_paths` 只影响显示或后续过滤，GraphEngine 扫描阶段未跳过排除目录 | 在 GraphEngine 扫描入口过滤 `excluded_dirs`，排除文件不再进入扫描和解析 |
| 32 | v4.6 | `stop-mindx.ps1` 在存在多个 TCP 连接时不能稳定停止服务 | PowerShell 把多个 `TIME_WAIT` TCP 连接当数组处理，端口判断和进程停止逻辑失真 | 修正 TIME_WAIT 数组处理，多个连接存在时仍能正确定位并停止 server |
| 33 | v4.6 | `/api/broken-links` 同一断链重复出现 | 外部递归解析和普通断链汇总同时记录同一 `(file, target)` | 按来源文件和目标去重后返回断链列表 |
| 34 | v4.6 | 外部断链在详情面板缺少静默按钮 | 前端只给内部断链渲染静默操作 | 断链渲染统一读取外部状态，内部和外部断链都显示 🔈/🔇 |
| 35 | v4.6 | broken 外部节点出现在引用树图、目录树图和依赖图 | 图视图未隔离 `external_status: broken` 节点 | 视觉视图过滤 broken 外部节点，断链只保留在诊断和详情中 |
